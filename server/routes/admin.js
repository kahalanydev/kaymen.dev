...
// GET /api/admin/security and GET /api/admin/analytics USED TO LIVE HERE.
//
// Both were retired on 2026-08-17 and replaced by dedicated routers:
//   server/routes/security.js  ->  /api/admin/security/*
//   server/routes/traffic.js   ->  /api/admin/traffic/*
//
// They are not deprecated aliases, they are gone. The security one could not
// have survived anyway - the new router is mounted at /api/admin/security in
// server/index.js AHEAD of this file, so a handler of the same name here would
// never be reached and would only look like it still worked.
//
// The analytics handler read every breakdown out of the raw visits and events
// tables. That is the thing the replacement had to stop doing: those tables are
// pruned on a retention window now (utils/rollup.js), so any figure computed
// from them silently covers less history every month. Breakdowns come from the
// permanent daily rollups instead.
