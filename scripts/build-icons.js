...
(async () => {
  ...
  try {
    ...
    /* square icons */
    const square = {};
    for (const px of [16, 32, 48, 180, 192, 512]) square[px] = await shoot(tileDeep, px, px, true);

    write('apple-touch-icon.png', square[180]);
    write('icon-192.png', square[192]);
    write('icon-512.png', square[512]);
    for (const px of [16, 32, 48, 180, 192, 512]) write(`${BRAND}/kaymen-icon-${px}.png`, square[px]);
